        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            <div class="card">
                <div class="card-header border-0 pt-6">
                    <h3 class="card-title">Editar Template de Mensagem</h3>
                </div>
                ...
                <div class="card-body pt-0 pb-6">
                    <form method="POST" action="{{ route('templates.update', $template->id) }}">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" name="nome" class="form-control" value="{{ old('nome', $template->nome) }}"
                                required>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <select name="conta_whatsapp_id" id="conta_whatsapp_id" class="form-select" required>
                                ...
                                @foreach ($contas as $conta)
                                    <option value="{{ $conta->id }}" data-api="{{ $conta->tipo_api }}" {{ old('conta_whatsapp_id', $template->conta_whatsapp_id) == $conta->id ? 'selected' : '' }}>
...
@section('script')
    <script>
        @php
            $selectedTemplate = old('template_meta', $template->template_name);
            $initialMap = old('variaveis', $template->variaveis->pluck('campo_origem', 'posicao'));
        @endphp
        ...
        const initialMap = {!! json_encode($initialMap) !!};
        ...
        function gerarCamposVariaveis(indices) {
            ...
            indices.forEach(i => {
                ...
                const inicial = initialMap[i];
                if (inicial) {
                    const badge = document.createElement('span');
                    badge.className = 'badge bg-primary text-white';
                    badge.textContent = inicial;
                    tagInput.appendChild(badge);
                    hidden.value = inicial;
                }
        ...
        contaSelect.addEventListener('change', function () {
            ...
            if (tipo === 'meta') {
                ...
                evolutionFields.classList.remove('d-none');
            }
        });
