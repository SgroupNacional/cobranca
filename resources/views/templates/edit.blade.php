        ...
        <div class="card-body pt-0">
            <form method="POST" action="{{ route('templates.update', $template->id) }}">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="nome" class="form-control" value="{{ old('nome', $template->nome) }}" required>
                </div>
...
@section('script')
    <script>
        ...
        const selectedTemplate = @json(old('template_meta', $template->template_name));
        const initialMap = @json(old('variaveis', $template->variaveis->pluck('campo_origem', 'posicao')));
